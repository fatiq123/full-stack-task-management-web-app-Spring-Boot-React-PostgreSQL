...
const EnhancedTaskList: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();
  const { tasks, loading, error, filteredTasks, totalPages } = useSelector((state: RootState) => state.tasks);
  ...
  const [lastFilter, setLastFilter] = useState<TaskFilterDto>({
    sortBy: 'dueDate',
    sortDirection: 'asc'
  });
  
  const handleFilter = (filter: TaskFilterDto) => {
    setLastFilter(filter);
    setFilterApplied(true);
    setPage(0);
    dispatch(filterTasks({ filter, page: 0, size }));
  };
  ...
  const handlePageChange = (event: React.ChangeEvent<unknown>, value: number) => {
    ...
    if (filterApplied) {
      dispatch(filterTasks({ filter: lastFilter, page: newPage, size }));
    }
  };
